...
export default function handler(req, res) {
  ...
  const selectGiftData = async () => {
    ...
    let data = await executeQuery(
      "select * from GiftList where UserID=? order by UserID DESC",
      [userLogin]
    );
    res.json(data);
  ...
  const dataUpdate = async () => {
    ...
    let data = await executeQuery(
      "update GiftList set state=1, GiverID=?, GiverName=? where id=?",
      [GiverID, GiverName, ItemID]
    );
    res.json(data);
  };
  ...
  const dataDelete = async () => {
    ...
    let data = await executeQuery("delete from GiftList where id=?", [ItemID]);
    res.json(data);
  };
